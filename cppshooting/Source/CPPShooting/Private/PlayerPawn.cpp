	...
	//흐르는 시간이 발사시간보다 커지면
		...
		if (currFiredBulletCount < 10) 
		{
			...
			if (currTime > fireTime)
			{
				...
				bulletRot = GetActorRotation();

				int32 angle = 360 / bulletCount;
				bulletRot.Roll += currFiredBulletCount * angle;
				MakeBullet(bulletPos, bulletRot);

				//발사 카운트 증가
				currFiredBulletCount++;
...
void APlayerPawn::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	//Skill_1 등록
	PlayerInputComponent->BindAction(TEXT("Skill_1"), IE_Released, this, &APlayerPawn::InputSkill_1);
	//Skill_2 등록
	PlayerInputComponent->BindAction(TEXT("Skill_2"), IE_Released, this, &APlayerPawn::InputSkill_2);
}
...
{	
	...
	bulletCount = currMode->currScore / 5 + 1;
	InputSkill_2();
}

void APlayerPawn::InputSkill_1()
{
	FVector bulletPos;
	FRotator bulletRot;
		...
		for (int32 i = 0; i < bulletCount; i++)
		{
			bulletPos = GetActorLocation();
			bulletRot = GetActorRotation();
			bulletRot.Roll += i * 360 / bulletCount;
			MakeBullet(bulletPos, bulletRot);
		}
...
void APlayerPawn::InputSkill_2()
{
	currFiredBulletCount = 0;
	return;
}
